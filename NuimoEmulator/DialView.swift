...
@IBDesignable
open class DialView : UIControl {
    @IBInspectable
    open var ringColor: UIColor = UIColor(colorLiteralRed: 0.25, green: 0.25, blue: 0.25, alpha: 1.0) { didSet { setNeedsDisplay() } }
    @IBInspectable
    open var knobColor: UIColor = UIColor(colorLiteralRed: 0.75, green: 0.75, blue: 0.75, alpha: 0.5) { didSet { setNeedsDisplay() } }
    @IBInspectable
    open var surfaceColor: UIColor = UIColor(colorLiteralRed: 0.5, green: 0.5, blue: 0.5, alpha: 1.0) { didSet { setNeedsDisplay() } }
    @IBInspectable
    open var ringSize: CGFloat = 40.0 { didSet { setNeedsDisplay() } }
    @IBInspectable
    open var handleSize: CGFloat = 50.0 { didSet { setNeedsDisplay() } }
    ...
    open override var isEnabled: Bool { didSet { setNeedsDisplay() } }

    @IBOutlet
    open var delegate: DialViewDelegate?

    /// Workaround for Xcode bug that prevents you from connecting the delegate in the storyboard.
    /// Remove this extra property once Xcode gets fixed.
    /// See also http://stackoverflow.com/a/35155533/543875
    @IBOutlet
    open var ibDelegate: AnyObject? {
        get { return delegate }
        set { delegate = newValue as? DialViewDelegate }
    }

    fileprivate var size: CGFloat { return min(frame.width, frame.height) }

    fileprivate var rotationSize: CGFloat { return size - max(handleSize, ringSize) }
